...
const CATEGORIAS_TABS = [
  ...
  { id: CATEGORIAS.BUZIOS, label: '🌴 Búzios', emoji: '🌴' },
  { id: CATEGORIAS.INTERNACIONAL, label: '🌍 Internacional', emoji: '🌍' },
  { id: CATEGORIAS.NACIONAL, label: '🏔️ Nacional', emoji: '🏔️' },
  { id: CATEGORIAS.PACKS, label: '🎁 Packs', emoji: '🎁' },
];
      ...
      <section className="section-pale-blue">
        <div className="container-moana">
          <div className="text-center mb-10">
            <h2 className="section-title">¿Por qué elegir Moana Turismo?</h2>
            <p className="section-subtitle">Tu agencia de confianza, con posada propia en Búzios</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {WHYS.map(({ icon: Icon, title, desc }, i) => (
              <div key={i} className="card p-6 text-center group hover:-translate-y-1 transition-transform duration-300">
                ...
                <h3 className="font-semibold text-moana-blue text-base mb-2">{title}</h3>
                <p className="text-moana-gray text-sm leading-relaxed">{desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <section className="section-white">
          <div className="container-moana">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
              <div>
                <h2 className="section-title">Destacados del Mes</h2>
        ...
        <div className="absolute inset-0 bg-moana-blue/75 flex items-center">
          <div className="container-moana text-white">
            <div className="max-w-lg">
              <span className="badge-teal mb-3 inline-block">🏡 Posada propia</span>
              <h2 className="font-display font-bold text-3xl md:text-4xl mb-3">
                Posada Moana en Búzios
              </h2>
              <p className="text-white/80 mb-5">
                A 600m de Rua das Pedras. Habitaciones con aire, TV, frigobar y desayuno continental.
                La casa de Búzios que siempre quisiste.
              </p>
              <Link to="/posada" className="btn-primary inline-flex items-center gap-2">
                Conocer la Posada <ArrowRight size={16} />
              </Link>
            </div>
          </div>
        </div>
